...
//  glClearColor(0.0,0.0,0.0,0.0); // always clear to black
 // glClearDepth(static_cast<GLclampf>(1.0));
  ...
  vtkInformation *tuProperties=vtkInformation::New();
  tuProperties->Set(vtkDepthPeelingPass::OpaqueZTextureUnit(),
    this->OpaqueZTexture->GetTextureUnit());
  tuProperties->Set(vtkDepthPeelingPass::TranslucentZTextureUnit(),
    this->TranslucentZTexture->GetTextureUnit());
  int destSize[2];
  destSize[0] = this->ViewportWidth;
  destSize[1] = this->ViewportHeight;
  tuProperties->Set(vtkDepthPeelingPass::DestinationSize(),destSize,2);
  int c = s->GetPropArrayCount();
  for (int i = 0; i < c; i++)
    {
    vtkProp *p=s->GetPropArray()[i];
    p->SetPropertyKeys(tuProperties);
    }
  tuProperties->Delete();
